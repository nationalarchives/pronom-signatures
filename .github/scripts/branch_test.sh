...
sed -i "s/assertThat(api\.getBinarySignatureVersion(), is(\"[0-9]\{3\}\"));/assertThat(api.getBinarySignatureVersion(), is(\"$(curl https://pronom.nationalarchives.gov.uk/signatures.json | jq '.latest_signature.version|tonumber'+1)\"));/"  droid/droid-api/src/test/java/uk/gov/nationalarchives/droid/internal/api/DroidAPITest.java
